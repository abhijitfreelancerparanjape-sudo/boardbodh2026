...
export function ReviewSelfAssess({
  ...
  // awards: rubric_component_id -> marks
  const [awards, setAwards] = useState<Record<string, number>>(() => {
    const init: Record<string, number> = {};
    for (const q of questions) for (const [cid, m] of Object.entries(q.selfScores)) init[cid] = m;
    return init;
  });
  const [finalTotal, setFinalTotal] = useState<number | null>(initialTotal);
  const [saving, setSaving] = useState(false);
  const [error, setError] = useState<string | null>(null);

  const totalComponents = useMemo(
    () => questions.reduce((s, q) => s + q.rubric.length, 0),
    [questions]
  );
  const markedComponents = Object.keys(awards).length;
  const runningTotal = Object.values(awards).reduce((s, m) => s + Number(m || 0), 0);
  const allMarked = markedComponents >= totalComponents && totalComponents > 0;
  ...
  // Final score view.
  if (finalTotal != null) {
    const pct = maxMarks > 0 ? Math.round((finalTotal / maxMarks) * 100) : 0;
    return (
      <main className="mx-auto max-w-xl px-6 py-16 text-center">
        ...
        <p className="mt-6 text-sm text-ink/55">
          Your weakest concepts will surface here in the next step.
        </p>
        <Link
          href="/dashboard"
          className="mt-6 inline-block rounded-full bg-terracotta px-6 py-3 text-sm font-medium text-bone hover:opacity-90"
        >
  ...
  // Self-assessment view (the gated reveal).
  return (
    <main className="mx-auto max-w-3xl px-6 py-12">
      <div className="rounded-2xl border border-sage/30 bg-sage/5 p-5 text-center">
        ...
        <p className="mt-1 text-sm text-ink/60">
          Your answers are locked. The marking scheme is now unlocked. Award yourself marks against
          each rubric point.
        </p>
        {late && (
      ...
      <div className="mt-6 flex items-center justify-between">
        <h1 className="font-display text-2xl text-ink">Score yourself</h1>
        <div className="text-right">
          ...
          <div className="text-xs text-ink/50">
            {markedComponents}/{totalComponents} points marked
          </div>
        </div>
      </div>
          ...
          <li key={q.attemptQuestionId} className="rounded-2xl border border-ink/10 bg-white/50 p-6">
            <div className="flex items-baseline justify-between gap-4">
              ...
              <span className="text-[10px] uppercase tracking-[0.14em] text-ink/45">
                {QUESTION_FORMAT_LABEL[q.format as keyof typeof QUESTION_FORMAT_LABEL] ?? q.format} ·{" "}
                {q.marks} marks
              </span>
            </div>
      ...
      >
        {saving ? "Saving…" : allMarked ? "See my score" : `Mark all ${totalComponents} rubric points`}
      </button>
    </main>
  );
}
